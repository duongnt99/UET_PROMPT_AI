  ...
  return (
    <form action={action} className="space-y-4">
      ...
      <div>
        ...
        <Input id="title" name="title" required defaultValue={challenge?.title} className="mt-1" />
      </div>
      <div>
        <Label htmlFor="prompt">Nội dung (cả hai đội nhận cùng đề)</Label>
        <Textarea id="prompt" name="prompt" required defaultValue={challenge?.prompt} className="mt-1 min-h-40" />
      </div>
      <div>
        <Label htmlFor="notes">Ghi chú nội bộ (không hiện sân khấu)</Label>
        <Textarea id="notes" name="notes" defaultValue={challenge?.notes} className="mt-1" />
      </div>
